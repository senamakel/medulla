...
`dir_context` folds into the capabilities probe prompt, and the suite then
...
can run the moment that infrastructure exists; they have not yet been exercised
against it.
...
- Routing is asserted on `taskId` correlation rather than a verbatim marker
  echo, because a real model will not parrot `TASKALPHA-123` back.
...
Reuse these when building a live harness for any TUI/daemon/CLI combination.
...
per process. The log file is greppable and is what `wait_for_regex` polls for
readiness. The rc file signals by its *existence* that the process exited, so poll
for it rather than calling `wait`. The pane, read with
`tmux capture-pane -p -t "$SESSION:<name>"`, covers anything that paints a TUI
instead of writing lines, and is the only way to assert on opencode's UI.
...
### 2. Poll for readiness markers rather than sleeping
...
A fixed sleep anywhere in this harness is the largest source of flakiness, so
there are none.
...
Output is deterministic and assertable: every completion is
...
The mock also keeps a request journal: every request appends one JSON line to
`llm.jsonl`. Assertions then check the *input* side too ("the task text appeared
in a chat request"), not only the visible output.
...
that reported more than the mock can observe would misstate the mock's blindness.

### 6. Hermetic HOME + kill auto-update

The real opencode CLI will **auto-update itself mid-test** and paint a blocking
"restart" dialog over the TUI, and an unsandboxed run leaks the updated binary
into `~/.opencode`. Defense in depth:
...
Any real third-party CLI in a harness needs its updater disabled, its HOME
sandboxed, and its version printed into the log (`opencode --version`) so drift is
visible in failures.
...
Over the host link the owner has to outlive a single leg. SSP state lives in
...
token-usage propagation, second round trip from a fresh owner identity, and the
...
### 9. Docker wraps the *same* script, with no second harness
...
- Runtime is `--network none` by default and passes, which proves the harness is
  fully loopback. Network is needed at build time only.
- Build natively (`--platform linux/$(uname -m ...)`); never force amd64 emulation
  on Apple Silicon, because qemu makes Rust builds and TUIs slow and flaky.
...
`capture-pane` of every window before exiting, so a failed CI run is debuggable from
...
1. Output leg: the terminal frame is `kind == "Reply"` and contains the marker.
   For `tests.sh`, `usage.inputTokens`/`outputTokens` must also be present, which
   guards against regressions in opencode's nested `tokens:{input,output}` usage
   shape.
...
- `cargo test` shows `e2e_daemon_providers::stdin_input_reaches_child_and_echoes_in_reply`
  failing for reasons unrelated to this suite (opencode stdin-echo drift). It is
  tracked separately.
