...
The transport between a medulla orchestrator and its remote hosts. It replaces the
tiny.place mailbox: instead of Signal-encrypting a frame, pushing it to a hosted
relay and polling it back down, endpoints exchange UDP datagrams carrying
...
|---|---|---|---|
| **Outer header** | forwarder key | backend + endpoints | routing, roaming, replay defence |
...
enrollment and determines the direction bit (§4.2). It is not a property of a
given datagram.
...
A single counter per **sending node**, incremented once per datagram sent,
regardless of destination. It starts at 1 and never repeats for the life of the
...
addressed to *it*, by *that* sender, at *that* sequence. A forwarder that
redirected a datagram to a different node would produce an authentication failure
...
`RTO = SRTT + 4·RTTVAR`, clamped to `[MIN_RTO, MAX_RTO]`, with SRTT/RTTVAR
maintained per peer from the `send_ts`/`reply_ts` samples in §4.1.
...
128 bits, generated on the **orchestrator**, per host. Encoded for a human to
retype:
...
- **A captured datagram replayed from a different source address does not
  rebind.** (§5 rule 5.)
