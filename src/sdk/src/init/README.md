...
- [`tests/`](./tests/) — Unit tests for workspace initialisation. Every test is offline and deterministic: the drafting path is exercised through a stub `ChatProvider` rather than a live model.
- [`draft.rs`](./draft.rs) — Drafting a workspace profile from a directory's instruction files.
...
- [`types.rs`](./types.rs) — Data types for workspace initialisation: the instruction files read from a directory, the profile fields an LLM drafts from them, and the outcome of a write.
