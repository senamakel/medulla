...
use crate::protocol::config::{load_config, TinyplaceFileConfig};
...
fn plant_identity(config_path: &Path) -> String {
    ...
    crate::protocol::config::write_config(config_path, &config).expect("write planted config");
    signer.agent_id()
}
...
fn a_bad_planted_key_is_reported_not_replaced() {
    ...
    let slot_one = medulla_home(root.path())
        .join("tinyplace")
        .join("config.json");
    crate::protocol::config::write_config(
        &slot_one,
        &TinyplaceFileConfig {
