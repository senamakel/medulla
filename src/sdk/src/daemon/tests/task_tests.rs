...
///
/// Each frame is handled by its own spawned task, so the sequential test above
/// only covers the easy ordering. With a separate `contains_key` check both
/// racing frames could pass it, the second would overwrite the first's
/// `RunningTask`, and the first admission guard's drop would then remove the
/// shared key — a still-running harness that no abort, input, or screen frame
/// can reach. Exactly one claim must win.
...
fn running_task_stub() -> crate::daemon::types::RunningTask {
    crate::daemon::types::RunningTask {
        provider: crate::protocol::HarnessProvider::Claude,
        accepts_stdin: false,
        correlation_id: None,
        stdin: None,
        pending_input: Vec::new(),
        session_id: None,
        abort: crate::daemon::providers::Abort::new(),
    }
}
