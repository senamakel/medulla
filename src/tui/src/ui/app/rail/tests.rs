...
// Cursor identity is kept here with the rail's other unit coverage: the rail
// directory owns both the rows and the cursor that selects them.
fn cursor_agent(id: &str) -> RailRow {
...
fn cursor_lane(key: &str) -> crate::ui::agents::AgentLane {
    crate::ui::agents::AgentLane {
        key: key.to_string(),
        label: String::new(),
        role: crate::ui::agents::AgentRole::Agent,
        turns: Vec::new(),
        last_at: 0,
        tasks: Vec::new(),
        context_tokens: None,
...
fn a_missing_anchor_uses_the_clamped_previous_offset() {
    let rows = vec![RailRow::NewAgent, cursor_agent("builder")];
...
fn an_overflow_anchor_uses_its_lanes_stable_key() {
    let lanes = vec![cursor_lane("builder")];
    ...
    let rows = vec![RailRow::NewAgent, cursor_agent("new"), overflow];
...
fn a_task_anchor_survives_local_pty_enrichment() {
    let lanes = vec![cursor_lane("builder")];
    let task = crate::ui::agents::TaskState {
        task_id: "t-1".to_string(),
        status: crate::ui::agents::TaskStatus::Running,
        turns: 0,
        last_at: 0,
        turn_blocks: Vec::new(),
        attention: None,
